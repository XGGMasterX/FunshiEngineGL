...
Las pruebas son headless (sin pila gráfica), corren con CTest y hay **13 targets**
(trece siempre + `scripts-java-tests` si el build encontró el JDK):

```bash
cmake --build build --target filemanager-tests configuracion-tests eventbus-tests menu-tests assetmanager-tests texturemanager-tests estructuras-tests scripts-tests scripts-runtime-tests manifiesto-assets-tests
